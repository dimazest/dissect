...
class Test(unittest.TestCase):
    ...
    def single_case(self, matrix_, expected, w):
    ...
    def single_case_raises(self, matrix_, error_type, w):
        dm = DenseMatrix(matrix_)
        sm = SparseMatrix(matrix_)
    ...
    def test_epmi(self):
        w = EpmiWeighting()
        test_cases = [(self.b, np.mat([[1, 1, 1]])), (self.c, self.c)]
        for matrix_, expected in test_cases:
            self.single_case(matrix_, expected, w)

    def test_plog(self):
        w = PlogWeighting()
        test_cases = [(np.mat([[1, 1, 1]]), np.mat([[0, 0, 0]])), (self.c, self.c)]
        for matrix_, expected in test_cases:
            self.single_case(matrix_, expected, w)
    ...
    def test_plmi(self):
        ...
        test_cases = [
            (self.b, np.mat([[0, 0, 0]])),
            (self.c, self.c),
            (self.e, np.mat([[0., 0.30830136, 0.46245204], [1.25276297, 0., 0.]]))
        ]
    ...
    def test_epmi_raises(self):
        ...
        for matrix_, error_type in test_cases:
            self.single_case_raises(matrix_, error_type, w)
